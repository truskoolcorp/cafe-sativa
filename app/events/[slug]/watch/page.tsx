...
type EventRow = {
  ...
  presenter_name: string | null; topic_brief: string | null
  access: 'ticketed' | 'free' | 'tier_included' | 'purchase_required'
}

const C = {
  bg:'#0d0a07', surface:'#161008', card:'#1c1409', border:'#2e1f0e',
  accent:'#b8813a', accentDim:'#7a5526', green:'#4caf7d', amber:'#e0a030',
  red:'#c0504a', text:'#e8ddd0', textDim:'#9e8870', muted:'#6b5540',
}
...
export default function WatchPage() {
    ...
    async function load() {
      try {
        ...
        const { data: ev, error: evErr } = await supabase
          .from('my_events_v')
          .select('id,slug,title,subtitle,starts_at,ends_at,status,zoom_join_url,zoom_meeting_id,presenter_name,topic_brief,access')
  ...
  const shell = (children: React.ReactNode) => (
    <div style={{ background:C.bg, minHeight:'100vh', fontFamily:"'Inter',sans-serif", color:C.text }}>
      <div style={{ borderBottom:`1px solid ${C.border}`, padding:'0 24px', display:'flex', alignItems:'center', height:52, background:C.surface }}>
        ...
        {event && (
          <span style={{ marginLeft:'auto', fontSize:11, fontWeight:700, padding:'3px 10px', borderRadius:10, textTransform:'uppercase', letterSpacing:'0.08em',
            background: event.status==='live' ? '#0a2010' : event.status==='ended' ? '#1a1410' : '#2a1f00',
            color: event.status==='live' ? C.green : event.status==='ended' ? C.muted : C.amber,
            border: `1px solid ${event.status==='live' ? C.green : event.status==='ended' ? C.border : C.amber}` }}>
            {event.status==='live' ? '● Live' : event.status==='ended' ? 'Ended' : `Starts in ${timeUntil||'...'}`}
          </span>
        )}
      </div>
  ...
  if (!isSignedIn) return shell(
    <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:'48px 40px', maxWidth:420, margin:'40px auto', textAlign:'center' }}>
  ...
  return shell(
    <>
      ...
      {/* Topic brief — shown when not yet live */}
      {!isLive && !isEnded && event.topic_brief && (
        <div style={{ background:'#1a1200', border:`1px solid ${C.accentDim}`, borderRadius:10, padding:'20px 24px', marginBottom:20 }}>
          <p style={{ margin:'0 0 8px', color:C.accent, fontSize:11, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase' }}>Tonight at the table</p>
          <p style={{ margin:0, color:C.textDim, fontSize:14, lineHeight:1.7 }}>{event.topic_brief}</p>
        </div>
      )}
      ...
      {isEnded ? (
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:'48px 40px', textAlign:'center', marginBottom:20 }}>
          ...
          <p style={{ margin:'0 0 24px', color:C.textDim, fontSize:14, lineHeight:1.6 }}>A recording will be available soon. Check back on The Stage for updates.</p>
        ...
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:'48px 40px', textAlign:'center', marginBottom:20 }}>
          ...
          <p style={{ margin:'0 0 8px', color:C.textDim, fontSize:14, lineHeight:1.6 }}>Your ticket is confirmed. This page updates automatically when we go live.</p>
          <p style={{ margin:'0 0 28px', color:C.muted, fontSize:13 }}>Keith will let you in from the waiting room — have your topic thoughts ready.</p>
          {event.zoom_join_url && (
            <a href={event.zoom_join_url} target="_blank" rel="noopener noreferrer"
              style={{ display:'inline-block', background:C.surface, border:`1px solid ${C.border}`, color:C.textDim, padding:'10px 22px', borderRadius:6, fontSize:13, textDecoration:'none' }}>
              Open in Zoom ↗
            </a>
          )}
        </div>
        ...
        <div style={{ marginBottom:20 }}>
          <div style={{ background:'#0a2010', border:`1px solid ${C.green}`, borderRadius:8, padding:'14px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
            <span style={{ color:C.green, fontSize:13, fontWeight:600 }}>● We are live — join the table now</span>
            <a href={event.zoom_join_url} target="_blank" rel="noopener noreferrer"
          ...
          {event.topic_brief && (
            <div style={{ background:'#1a1200', border:`1px solid ${C.accentDim}`, borderRadius:8, padding:'16px 20px', marginBottom:16 }}>
              <p style={{ margin:'0 0 6px', color:C.accent, fontSize:11, fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase' }}>Tonight at the table</p>
              <p style={{ margin:0, color:C.textDim, fontSize:13, lineHeight:1.6 }}>{event.topic_brief}</p>
            </div>
          )}
          ...
          <p style={{ margin:'10px 0 0', color:C.muted, fontSize:12, textAlign:'center' }}>If the embed does not load, use Open in Zoom above.</p>
        </div>
      ) : (
        <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:12, padding:'48px 40px', textAlign:'center', marginBottom:20 }}>
      ...
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:8, padding:'14px 20px', display:'flex', alignItems:'center', gap:10 }}>
        ...
        <span style={{ color:C.textDim, fontSize:13 }}>Ticket confirmed · <Link href={`/events/${slug}`} style={{ color:C.accent, textDecoration:'none' }}>Event details</Link></span>
      </div>
    </>
  )
}
