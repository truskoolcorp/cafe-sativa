...
export default function CafeSativaHomepage() {
  ...
  const tiers = [
    ...
    },
    {
      ...
      features: [
        ...
        'VIP-only digital experiences',
        'Priority reservation status for future physical openings',
        'Exclusive invites and perks',
      ],
  ...
  async function handleWaitlistSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage('You are on the list. Welcome to Café Sativa.')
    ...
    <main className="min-h-screen bg-[#0d0a09] text-[#f7efe4]">
      <div className="relative overflow-hidden">
        ...
        <header className="relative z-10 mx-auto flex max-w-7xl items-center justify-between px-6 py-6 lg:px-10">
          <div>
            <Link href="/" className="text-xs uppercase tracking-[0.35em] text-[#d7a25e]">
              Café Sativa
            </Link>
          ...
          <a
            href="#waitlist"
            className="rounded-full border border-[#c8843c] bg-[#c8843c]/10 px-5 py-2 text-sm font-medium text-[#ffd7ad] transition hover:bg-[#c8843c] hover:text-[#1c120d] md:hidden"
          >
            Join the Guest List
          </a>
        </header>
          ...
          <div className="rounded-[2rem] border border-white/10 bg-[linear-gradient(135deg,#1b120d,#23160f)] p-7 shadow-2xl shadow-black/40">
            <div className="rounded-[1.5rem] border border-[#d1914d]/30 bg-black/20 p-6">
              ...
              <div className="mt-6 flex flex-col gap-3 sm:flex-row">
                ...
                <Link href="/membership" className="rounded-full border border-white/15 bg-white/5 px-6 py-3 text-center text-sm font-semibold uppercase tracking-[0.22em] text-[#f3e4d3] transition hover:bg-white/10">
                  Join as Insider
                </Link>
              </div>
            </div>
            ...
            <form className="space-y-5" onSubmit={handleWaitlistSubmit}>
              ...
              <div>
                ...
                <select
                  ...
                  defaultValue="Featured chef drops"
                >
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full rounded-full bg-[#d08e49] px-6 py-4 text-sm font-semibold uppercase tracking-[0.22em] text-[#1c120d] transition hover:bg-[#eea75a] disabled:opacity-50"
              >
                {submitting ? 'Joining...' : 'Join the Guest List'}
              </button>
              {message && <p className="text-sm leading-6 text-[#d6c0a6]">{message}</p>}
